...
public class UserProductsDaoBean implements UserProductsDao {
    ...
    @Override
    public Optional<UserProductsDto> findUserProductNameInUserDto(String name, String id) {
        TypedQuery<UserProductsDto> query = entityManager.createQuery("SELECT new com.infoshare.fourfan.dto.UserProductsDto(" +
                "p.id, p.useridInt, p.product.name, p.product.brand, p.product.price, p.product.calories, p.product.shop.shop, p.product.productCategory.category, p.amount) FROM UserProducts p WHERE p.product.name = :nameParm and p.useridInt = :idParm", UserProductsDto.class);
